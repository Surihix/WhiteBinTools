...
namespace WhiteBinTools.src.Repack
{
    internal class BinRpkMoreFiles
    {
        ...
        public static void RepackMoreFiles(CmnEnums.GameCodes gameCodeVar, string filelistFileVar, string whiteBinFileVar, string extractedDirVar)
        {
            using (var logStream = new FileStream("ProcessLog.txt", FileMode.Append, FileAccess.Write, FileShare.Write))
            {
                using (var logWriter = new StreamWriter(logStream))
                {
                    ...
                    try
                    {
                                                                ...
                                                                case true:
                                                                    ...
                                                                    using (var cmpDataStream = new FileStream(tmpCmpDataFile, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite))
                                                                    {
                                                                        ...
                                                                        if (newCmpSize < ogCSize || newCmpSize.Equals(ogCSize))
                                                                        {
                                                                            ...
                                                                            using (FileStream injectWhiteBin = new FileStream(whiteBinFileVar, FileMode.Open, FileAccess.Write))
                                                                            {
                        ...
                        Console.WriteLine("\nFinished repacking files to " + whiteBinFileVar);
                        logWriter.WriteLine("\nFinished repacking files to " + whiteBinFileVar);
                    }
                    catch (Exception ex)
                    {
