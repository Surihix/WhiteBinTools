...
namespace WhiteBinTools
{
    internal class BinRepack
    {
        ...
        public static void Repack(string GameCode, string FilelistFile, string Extracted_Dir)
        {
                    ...
                    try
                    {
                                            ...
                                            using (FileStream UpdChunk = new FileStream(NewChunkFile + ChunkFNameCount, FileMode.Append, FileAccess.Write))
                                            {
                                                using (StreamWriter UpdChunkStrings = new StreamWriter(UpdChunk))
                                                {
                                                    ...
                                                    for (int f = 0; f < FilesInChunkCount; f++)
                                                    {
                                                        ...
                                                        using (FileStream WhiteBin = new FileStream(WhiteBinFile, FileMode.Append, FileAccess.Write))
                                                        {
                                                            ...
                                                            using (FileStream PackAFile = new FileStream(FullFilePath, FileMode.Open, FileAccess.Read))
                                                            {
                                                                WhiteBin.Seek(FilePos, SeekOrigin.Begin);
                                                                ...
                                                                switch (CompressedState)
                                                                {
                                                                    case true:
                                                                        ...
                                                                        using (FileStream CmpDataStream = new FileStream(TmpCmpDataFile, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
                                                                        {
                        ...
                        if (GameCode.Equals("-ff132"))
                        {
                            if (!UnEncryptedFilelists.Contains(FilelistName))
                            {
                                ...
                                CmnMethods.FFXiiiCryptTool(InFilelistFileDir, " -c ", "\"" + FilelistFile + "\"", ref CheckSumActionArg);
                                ...
                                CmnMethods.FFXiiiCryptTool(InFilelistFileDir, " -e ", "\"" + FilelistFile + "\"", ref CryptFilelistCode);
