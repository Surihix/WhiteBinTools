    ...
    public class BinUnpkAFile {
        ...
        public static void UnpackFile(int GameCode, string FilelistFile, string WhiteBinFile, string WhiteFilePath) {
            ...
            if (!File.Exists(WhiteBinFile)) {
                Utils.LogMsgs("Error: Image bin file specified in the argument is missing");
                Utils.ErrorExit("");
            }
            ...
            Utils.IfFileExistsDel(FilelistFile + ".bak");
            ...
            try {
                        ...
                        using (BinaryReader ChunkStringReader = new BinaryReader(CurrentChunk)) {
                            ...
                            for (int f = 0; f < FilesInChunkCount; f++) {
                                ...
                                if (MainPath.Equals(WhiteFilePath)) {
                                    ...
                                    lock (_lockObject) {
                                        Utils.LogMsgs(UnpackedState + " " + Extract_dir_Name + "\\" + MainPath);
                                    }
                                }
                ...
                lock (_lockObject) {
                    if (HasExtracted.Equals(false)) {
                        Utils.LogMsgs("Specified file does not exist. please specify the correct file path");
                        Console.WriteLine("");
                        ...
                        Console.WriteLine("Finished extracting file " + WhiteBinName);
                        if (CountDuplicate > 1) {
                            Utils.LogMsgs(CountDuplicate + " duplicate file(s)");
                        }
                    }
                }
            }
